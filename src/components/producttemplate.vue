...
<section class="text-gray-900  body-font overflow-hidden px-20">
  ...
  <div class="container py-8 mx-auto">
    <div class="lg:w-4/5 mx-auto flex flex-wrap">
      <img class="lg:w-1/2 w-full lg:h-auto h-64 object-cover object-center rounded" v-bind:src="$baseURL + '/image/get/' + product.product_image">
      ...
      <!-- <input type="date" name="manufacturer-date" id="manufacturer-date" class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm border border-gray-400 rounded-md"> -->
        ...
        <div class="flex mt-6 items-center pb-5 border-b-2 border-gray-100 mb-5">
          <div class="flex">
            ...
            <button v-if="product.product_colors.filter(color => {return color.color_id=='1'}).length" @click="handleColor('red')" :class="{'border-green-500': color=='red'}" class="border-2 border-gray-300 ml-1 bg-red-500 rounded-full w-6 h-6 focus:outline-none"></button>
            <button v-if="product.product_colors.filter(color => {return color.color_id=='2'}).length"  @click="handleColor('blue')" :class="{'border-green-500': color=='blue'}" class="border-2 border-gray-300 ml-1 bg-blue-500 rounded-full w-6 h-6 focus:outline-none"></button>
            <button v-if="product.product_colors.filter(color => {return color.color_id=='3'}).length"  @click="handleColor('green')" :class="{'border-green-500': color=='green'}" class="border-2 border-gray-300 ml-1 bg-green-500 rounded-full w-6 h-6 focus:outline-none"></button>
            <button v-if="product.product_colors.filter(color => {return color.color_id=='4'}).length"  @click="handleColor('yellow')" :class="{'border-green-500': color=='yellow'}" class="border-2 border-gray-300 ml-1 bg-yellow-400 rounded-full w-6 h-6 focus:outline-none"></button>
            <button v-if="product.product_colors.filter(color => {return color.color_id=='5'}).length"  @click="handleColor('purple')" :class="{'border-green-500': color=='purple'}" class="border-2 border-gray-300 ml-1 bg-purple-500 rounded-full w-6 h-6 focus:outline-none"></button>
            <button v-if="product.product_colors.filter(color => {return color.color_id=='6'}).length"  @click="handleColor('white')" :class="{'border-green-500': color=='white'}" class="border-2 border-gray-300 ml-1 bg-white rounded-full w-6 h-6 focus:outline-none"></button>
            <button v-if="product.product_colors.filter(color => {return color.color_id=='7'}).length"  @click="handleColor('pink')" :class="{'border-green-500': color=='pink'}" class="border-2 border-gray-300 ml-1 bg-pink-500 rounded-full w-6 h-6 focus:outline-none"></button>
            <button v-if="product.product_colors.filter(color => {return color.color_id=='8'}).length"  @click="handleColor('orange')" :class="{'border-green-500': color=='orange'}" class="border-2 border-gray-300 ml-1 bg-yellow-600 rounded-full w-6 h-6 focus:outline-none"></button>
            <button v-if="product.product_colors.filter(color => {return color.color_id=='9'}).length"  @click="handleColor('black')" :class="{'border-green-500': color=='black'}" class="border-2 border-gray-300 ml-1 bg-black rounded-full w-6 h-6 focus:outline-none"></button>
          </div>
        </div>
        <div class="flex ">
        ...
        <button class="flex-initial ml-4 text-white  bg-red-500 border-0 py-2 px-6 focus:outline-none hover:bg-red-600 rounded" @click="handleEditProduct">Edit</button>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    handleEditProduct() {
        this.$emit("handleEditProductEmit", this.product);
    },
    handleColor(color) {
      this.color = color
    },
